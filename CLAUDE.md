...
- 🤖 Ask-AI side panel on lesson pages (`site/ai-assistant.{js,css}`, copied to `dist/ai-assistant.js`): reader supplies their own API key (localStorage only), browser calls the provider directly; 19 presets grouped 国际/国内/本地/聚合 (Anthropic, OpenAI, Gemini, xAI, Mistral, Groq, DeepSeek, Qwen, Kimi, GLM, 豆包, MiniMax, 混元, 文心, Ollama, LM Studio, OpenRouter, SiliconFlow, custom) plus a ⟳ button that fetches `{base}/models` live. First run shows a 3-step wizard (3 recommended providers per UI language from `RECOMMEND`, link to the provider's `keyUrl`, paste key, one test request before saving). Config is v2: several providers can be configured at once (`cfg.profiles`, one active), switched from the header menu. Context scope (📎 row above the input): auto (the numbered `## N.` grammar point on screen, default), whole lesson, or one grammar point; quick prompts always send the whole lesson; selecting text inside a grammar point pins the scope to it, and the quoted text is cleaned (no furigana / 🔊, trimmed to the Japanese sentence). History lives in localStorage per lesson (24 msgs kept, last 6 turns sent). Replies render `漢字（かな）` as ruby; thinking/reasoning deltas show in a collapsible block; answers have copy / regenerate, errors have retry
- Security for the AI panel: every page carries a `<meta http-equiv="Content-Security-Policy">` with a `connect-src` allowlist (self, GA, provider hosts parsed from `site/ai-assistant.js`, localhost/127.0.0.1). Adding a provider = adding its `base:` (and optional `altBase:`) URL in the JS; the CSP picks both up at build time. Preset model IDs were last verified against official docs on 2026-09-16; the ⟳ button fetches the live list. `effortParams()` maps the 快速/标准/深入 setting onto each provider's reasoning knob. Settings only accept https or local base URLs, warn on non-official hosts, and offer session-only key storage. Privacy explanation lives in `pages/about.md` (#ai-privacy)
...
5. **Audio tagging**: Assigns **positional** `data-audio="l{lessonNum}.{seq}"` IDs to audio-eligible `<li data-ja>` elements in document order, and writes the id→text list to `dist/.audio-requests.json` (consumed by `build-audio.py`). ⚠️ IDs are positional, so inserting/removing examples shifts every later ID in that lesson — its mp3s must be regenerated. (`audio/manifest.json` is legacy and no longer read by the build.)
